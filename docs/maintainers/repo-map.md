# AgentZero Repository Map

AgentZero is a Rust-first autonomous agent runtime. It receives messages from messaging platforms, routes them through an LLM, executes tool calls, persists memory, and returns responses. It can also control hardware peripherals and run as a long-lived daemon.
...
| File | Lines | Role |
|---|---|---|
| `main.rs` | 1,977 | CLI entrypoint. Clap parser, command dispatch. All `agentzero <subcommand>` routing lives here. |
