# AgentZero on Arduino Uno Q — Step-by-Step Guide

Run AgentZero on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

AgentZero includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `agentzero peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
## Phase 2: Install AgentZero on Uno Q
...
# Clone agentzero (or scp your project)
git clone https://github.com/myagentzero/agentzero.git
cd agentzero
...
# Install
sudo cp target/release/agentzero /usr/local/bin/
```
...
# Copy to Uno Q
scp target/aarch64-unknown-linux-gnu/release/agentzero arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/agentzero /usr/local/bin/"
```
...
# Quick config
agentzero onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Or create config manually
mkdir -p ~/.agentzero/workspace
nano ~/.agentzero/config.toml
```
...
**At this point:** Telegram chat works. Send messages to your bot — AgentZero responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (AgentZero Handles It)

AgentZero includes the Bridge app and setup command.

### 5.1 Deploy Bridge App

**From your Mac** (with agentzero repo):
```bash
...
```bash
agentzero peripheral setup-uno-q
```
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, AgentZero uses `gpio_write` via the Bridge.
...
| 5 | `git clone https://github.com/myagentzero/agentzero.git && cd agentzero` |
| 6 | `cargo build --release --features hardware` |
| 7 | `agentzero onboard --api-key KEY --provider openrouter` |
| 8 | Edit `~/.agentzero/config.toml` (add Telegram bot_token) |
| 9 | `agentzero daemon --host 127.0.0.1 --port 42617` |
...
- **"command not found: agentzero"** — Use full path: `/usr/local/bin/agentzero` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`agentzero peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. AgentZero uses the correct v4 endpoint.
