# AgentZero on Nucleo-F401RE — Step-by-Step Guide

Run AgentZero on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

AgentZero can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
AgentZero includes everything for Nucleo-F401RE:
...
| Flash command | `agentzero peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via AgentZero

From the agentzero repo root:
...
```bash
agentzero agent --message "Turn on the LED on pin 13"
```
...
| 3 | `agentzero peripheral flash-nucleo` |
| 4 | Add Nucleo to config.toml (path = your serial port) |
| 5 | `agentzero daemon` or `agentzero agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `agentzero peripheral list` to verify.
