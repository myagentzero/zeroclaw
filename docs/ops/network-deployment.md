# Network Deployment — AgentZero on Raspberry Pi and Local Network

This document covers deploying AgentZero on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | AgentZero polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | AgentZero syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — AgentZero connects to external servers/relays. No port forwarding or public IP required.

---

## 2. AgentZero on Raspberry Pi
...
```bash
agentzero daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: AgentZero polls Telegram API (outbound)
...
```bash
agentzero daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `agentzero tunnel` (see tunnel docs).

3. AgentZero will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- AgentZero calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `agentzero daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `agentzero daemon` service).
...
- [ ] Run `agentzero daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
AgentZero supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `agentzero` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo agentzero service install
```

This creates:
- Init script: `/etc/init.d/agentzero`
- Config directory: `/etc/agentzero/`
- Log directory: `/var/log/agentzero/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo agentzero service install` automatically prepares `/etc/agentzero`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `agentzero` service user.

If no prior runtime state is available to migrate, create `/etc/agentzero/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add agentzero default

# Start the service
sudo rc-service agentzero start

# Check status
sudo rc-service agentzero status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service agentzero start` | Start the daemon |
| `sudo rc-service agentzero stop` | Stop the daemon |
| `sudo rc-service agentzero status` | Check service status |
| `sudo rc-service agentzero restart` | Restart the daemon |
| `sudo agentzero service status` | AgentZero status wrapper (uses `/etc/agentzero` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/agentzero/access.log` |
| Errors/stderr | `/var/log/agentzero/error.log` |

View logs:

```bash
sudo tail -f /var/log/agentzero/error.log
```
...
sudo rc-service agentzero stop
sudo rc-update del agentzero default

# Remove init script
sudo agentzero service uninstall
```
...
- The service runs as the `agentzero:agentzero` user (least privilege)
- Config must be at `/etc/agentzero/config.toml` (explicit path in init script)
- If the `agentzero` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo agentzero service install`
- [ ] Enable: `sudo rc-update add agentzero default`
- [ ] Start: `sudo rc-service agentzero start`
- [ ] Verify: `sudo rc-service agentzero status`
- [ ] Check logs: `/var/log/agentzero/error.log`
