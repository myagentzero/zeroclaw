...
- `cargo check` / `cargo build` appears stuck at `Checking agentzero` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in AgentZero:
...
### `agentzero` command not found after install

Symptom:

- install succeeds but shell cannot find `agentzero`
...
- stop extra `agentzero daemon` / `agentzero channel start` processes
