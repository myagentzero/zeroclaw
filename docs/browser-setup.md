...
This guide covers setting up browser automation capabilities in AgentZero, including both headless automation and GUI access via VNC.

## Overview

AgentZero supports multiple browser access methods:
...
```bash
echo "Open https://example.com and tell me what it says" | agentzero agent
```
...
### AgentZero Integration Tests

```bash
# Content extraction
echo "Open https://example.com and summarize it" | agentzero agent

# Navigation
echo "Go to https://github.com/trending and list the top 3 repos" | agentzero agent

# Form interaction
echo "Go to Wikipedia, search for 'Rust programming language', and summarize" | agentzero agent
```
...
- [AgentZero Configuration Reference](./config-reference.md)
