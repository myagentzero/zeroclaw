...
This guide covers building and installing AgentZero on Windows.
...
```powershell
scoop bucket add agentzero https://github.com/myagentzero/scoop-agentzero
scoop install agentzero
```
...
copy target\x86_64-pc-windows-msvc\release\agentzero.exe %USERPROFILE%\.agentzero\bin\
```
...
AgentZero uses Cargo feature flags to control which integrations are compiled in:
...
2. **Initialize AgentZero:**
   ```cmd
   agentzero init
   ```
3. **Configure your API key** in `%USERPROFILE%\.agentzero\config.toml`
