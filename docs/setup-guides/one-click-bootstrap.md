...
This page defines the fastest supported path to install and initialize AgentZero.
...
Default behavior is **app-only** (build/install AgentZero) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/myagentzero/agentzero/master/install.sh | bash
```
...
This builds a local AgentZero image and launches onboarding inside a container while
persisting config/workspace to `./.agentzero-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
the installer auto-falls back to `podman`. You can also set `AGENTZERO_CONTAINER_CLI`
explicitly (for example: `AGENTZERO_CONTAINER_CLI=podman ./install.sh --docker`).
...
Docker tag (`AGENTZERO_DOCKER_IMAGE`, default: `agentzero-bootstrap:local`); if missing,
it pulls `ghcr.io/myagentzero/agentzero:latest` and tags it locally before running.
...
are persisted in the data directory (default: `./.agentzero-docker`, or `~/.agentzero-docker`
when bootstrapping via `curl | bash`). You can override this path with `AGENTZERO_DOCKER_DATA_DIR`.
...
podman run -d --name agentzero \
  --restart unless-stopped \
  --userns keep-id \
  --user "$(id -u):$(id -g)" \
  -v "$PWD/.agentzero-docker/.agentzero:/agentzero-data/.agentzero:Z" \
  -v "$PWD/.agentzero-docker/workspace:/agentzero-data/workspace:Z" \
  -e HOME=/agentzero-data \
  -e AGENTZERO_WORKSPACE=/agentzero-data/workspace \
  -p 42617:42617 \
  agentzero-bootstrap:local \
...
# Remove the container (data in volumes/.agentzero-docker is preserved)
docker rm agentzero
...
saved in `.agentzero-docker/.agentzero/config.toml` and do not need to be passed again.
...
```bash
AGENTZERO_API_KEY="sk-..." AGENTZERO_PROVIDER="openrouter" ./install.sh
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/myagentzero/agentzero:latest`)
- `--skip-install`
- `--provider <id>`
