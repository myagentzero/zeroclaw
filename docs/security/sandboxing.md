# AgentZero Sandboxing Strategies
...
## Problem
AgentZero currently has application-layer security (allowlists, path blocking, command injection protection) but lacks OS-level containment. If an attacker is on the allowlist, they can run any allowed command with agentzero's user permissions.
...
readwrite_paths = ["$HOME/workspace", "/tmp/agentzero"]
```
