...
[INFO] ✓ Audit logging enabled (~/.config/agentzero/audit.log)
...
```bash
# Check what's active
$ agentzero security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/agentzero/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ agentzero config set security.sandbox.enabled false

# Enable specific backend
$ agentzero config set security.sandbox.backend firejail

# Adjust limits
$ agentzero config set security.resources.max_memory_mb 2048
```
...
✅ **Status visibility** — `agentzero security --status`
