...
3. Generic fallback env vars: `AGENTZERO_API_KEY` then `API_KEY`
...
- After multimodal normalization, AgentZero sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, AgentZero returns a structured capability error instead of silently ignoring images.
...
- AgentZero normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `agentzero models refresh --provider llamacpp`
...
- Model discovery: `agentzero models refresh --provider sglang`
...
- Model discovery: `agentzero models refresh --provider vllm`
...
- Model discovery: `agentzero models refresh --provider osaurus`
...
- Model discovery: `agentzero models refresh --provider nvidia`
