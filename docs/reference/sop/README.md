...
- CLI `agentzero sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.agentzero/workspace/sops/deploy-prod/SOP.toml
   ~/.agentzero/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   agentzero sop list
   agentzero sop validate
   agentzero sop show deploy-prod
   ```
