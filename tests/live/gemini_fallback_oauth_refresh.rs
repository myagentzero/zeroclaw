...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.agentzero/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.agentzero/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let agentzero_dir = PathBuf::from(home).join(".agentzero");
    let auth_profiles_path = agentzero_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: agentzero auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: agentzero auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.agentzero/auth-profiles.json (with expired token)
    let provider = agentzero::providers::create_provider("gemini", None)?;
