...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/agentzero-pacman.XXXXXX.conf)"
...
usage() {
  cat <<'USAGE'
AgentZero installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds AgentZero, configures your provider and API key,
...
Examples:
  ...
  curl -fsSL https://agentzerolabs.ai/install.sh | bash
...
Environment:
  AGENTZERO_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  AGENTZERO_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  AGENTZERO_DOCKER_IMAGE      Docker image tag to build/run (default: agentzero-bootstrap:local)
  AGENTZERO_API_KEY           Used when --api-key is not provided
  AGENTZERO_PROVIDER          Used when --provider is not provided (default: openrouter)
  AGENTZERO_MODEL             Used when --model is not provided
  AGENTZERO_CARGO_FEATURES    Extra cargo features for source builds (comma/space separated)
  AGENTZERO_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  AGENTZERO_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  AGENTZERO_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${AGENTZERO_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${AGENTZERO_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/myagentzero/agentzero/releases"
...
install_prebuilt_binary() {
  ...
  asset_name="agentzero-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/myagentzero/agentzero/releases/latest/download/${asset_name}"
  fi

  temp_dir="$(mktemp -d -t agentzero-prebuilt-XXXXXX)"
  ...
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f \( -name agentzero -o -name agentzero \) -perm -u+x | head -n 1 || true)"
  fi
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/agentzero-pacman.XXXXXX.conf)"
...
MSG
      ...
      _xcode_test_file="$(mktemp /tmp/agentzero-xcode-check.XXXXXX.c)"
...
prompt_api_key() {
  ...
  if [[ -n "$api_key_input" ]]; then
    ...
    warn "No API key entered — you can configure it later with agentzero onboard"
    SKIP_ONBOARD=true
  fi
}
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} AgentZero Guided Installer${RESET}"
...
ensure_default_config_and_workspace() {
  ...
  #
  # $1 — config directory  (e.g. ~/.agentzero or $docker_data_dir/.agentzero)
  # $2 — workspace directory (e.g. ~/.agentzero/workspace or $docker_data_dir/workspace)
...
# AgentZero configuration — generated by install.sh
# Edit this file or run 'agentzero onboard' to reconfigure.
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${AGENTZERO_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set AGENTZERO_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set AGENTZERO_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${AGENTZERO_DOCKER_IMAGE:-agentzero-bootstrap:local}"
  fallback_image="ghcr.io/myagentzero/agentzero:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.agentzero-docker"
  else
    default_data_dir="$WORK_DIR/.agentzero-docker"
  fi
  docker_data_dir="${AGENTZERO_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.agentzero" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official AgentZero image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.agentzero:/agentzero-data/.agentzero"
  workspace_mount="$docker_data_dir/workspace:/agentzero-data/workspace"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run agentzero onboard inside the container to configure."
  fi
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/myagentzero/agentzero.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${AGENTZERO_CONTAINER_CLI:-docker}"
API_KEY="${AGENTZERO_API_KEY:-}"
PROVIDER="${AGENTZERO_PROVIDER:-openrouter}"
MODEL="${AGENTZERO_MODEL:-}"
CARGO_FEATURES_INPUT="${AGENTZERO_CARGO_FEATURES:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${AGENTZERO_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set AGENTZERO_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t agentzero-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} AgentZero Installer${RESET}"
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing AgentZero installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing AgentZero installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized AgentZero data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}agentzero status${RESET}"
  echo -e "  ${DIM}agentzero agent -m \"Hello, AgentZero!\"${RESET}"
  echo -e "  ${DIM}agentzero gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.agentzerolabs.ai/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing AgentZero${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing AgentZero v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  ...
  # Clean up stale cargo install tracking from the old "agentzero" package name
  # (renamed to "agentzerolabs"). Without this, `cargo install agentzerolabs` from
  # crates.io fails with "binary already exists as part of `agentzero`".
  if have_cmd cargo; then
    if [[ -f "$HOME/.cargo/.crates.toml" ]] && grep -q '^"agentzero ' "$HOME/.cargo/.crates.toml" 2>/dev/null; then
      step_dot "Removing stale cargo tracking for old 'agentzero' package name"
      cargo uninstall agentzero 2>/dev/null || true
    fi
  fi

  cargo install --path "$WORK_DIR" --force --locked "${CARGO_FEATURE_ARGS[@]}"
  step_ok "AgentZero installed"
...
DESKTOP_DOWNLOAD_URL="https://www.agentzerolabs.ai/download"
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  ...
  case "$OS_NAME" in
    Darwin)
      if [[ -d "/Applications/AgentZero.app" ]] || [[ -d "$HOME/Applications/AgentZero.app" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (AgentZero.app)"
      fi
      ;;
    Linux)
      if have_cmd agentzero-desktop; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (agentzero-desktop)"
      elif [[ -x "$HOME/.local/bin/agentzero-desktop" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (~/.local/bin/agentzero-desktop)"
      fi
      ;;
  esac
  ...
  if [[ "$DESKTOP_APP_DETECTED" == false ]]; then
    ...
    echo -e "  Menu bar access to your AgentZero agent."
    ...
    echo -e "  ${DIM}Or run: agentzero desktop --install${RESET}"
  fi
...
if [[ -x "$HOME/.cargo/bin/agentzero" ]]; then
  AGENTZERO_BIN="$HOME/.cargo/bin/agentzero"
elif [[ -x "$WORK_DIR/target/release/agentzero" ]]; then
  AGENTZERO_BIN="$WORK_DIR/target/release/agentzero"
elif have_cmd agentzero; then
  AGENTZERO_BIN="agentzero"
elif [[ -x "$HOME/.cargo/bin/agentzero" ]]; then
  AGENTZERO_BIN="$HOME/.cargo/bin/agentzero"
elif [[ -x "$WORK_DIR/target/release/agentzero" ]]; then
  AGENTZERO_BIN="$WORK_DIR/target/release/agentzero"
...
if [[ "$SKIP_ONBOARD" == false && -n "$AGENTZERO_BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    step_dot "Configuring provider: ${PROVIDER}"
    ONBOARD_CMD=("$AGENTZERO_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    if "${ONBOARD_CMD[@]}" 2>/dev/null; then
      ...
      step_fail "Provider configuration failed — run agentzero onboard to retry"
    fi
  elif [[ "$PROVIDER" == "ollama" ]]; then
    step_dot "Configuring Ollama (no API key needed)"
    if "$AGENTZERO_BIN" onboard --provider ollama 2>/dev/null; then
      step_ok "Ollama configured"
    else
      step_fail "Ollama configuration failed — run agentzero onboard to retry"
    fi
  else
    ...
    if [[ -t 0 && -t 1 ]]; then
      ...
      step_dot "No API key provided — run agentzero onboard to configure"
    fi
  fi
elif [[ "$SKIP_ONBOARD" == true ]]; then
  step_dot "Skipping configuration (run agentzero onboard later)"
elif [[ -z "$AGENTZERO_BIN" ]]; then
  warn "AgentZero binary not found — cannot configure provider"
fi
...
_native_config_dir="${AGENTZERO_CONFIG_DIR:-$HOME/.agentzero}"
_native_workspace_dir="${AGENTZERO_WORKSPACE:-$_native_config_dir/workspace}"
ensure_default_config_and_workspace "$_native_config_dir" "$_native_workspace_dir" "$PROVIDER"

# --- Gateway service management ---
if [[ -n "$AGENTZERO_BIN" ]]; then
  # Try to install and start the gateway service
  step_dot "Checking gateway service"
  if "$AGENTZERO_BIN" service install 2>/dev/null; then
    step_ok "Gateway service installed"
    if "$AGENTZERO_BIN" service restart 2>/dev/null; then
      step_ok "Gateway service restarted"

    else
      step_fail "Gateway service restart failed — re-run with agentzero service start"
    fi
  else
    step_dot "Gateway service not installed (run agentzero service install later)"
  fi

  # --- Post-install doctor check ---
  step_dot "Running doctor to validate installation"
  if "$AGENTZERO_BIN" doctor 2>/dev/null; then
    step_ok "Doctor complete"
  else
    warn "Doctor reported issues — run agentzero doctor --fix to resolve"
  fi
fi
...
INSTALLED_VERSION=""
if [[ -n "$AGENTZERO_BIN" ]]; then
  INSTALLED_VERSION="$("$AGENTZERO_BIN" --version 2>/dev/null | awk '{print $NF}' || true)"
fi

# --- Success banner ---
echo
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} AgentZero installed successfully (AgentZero ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} AgentZero installed successfully!${RESET}"
fi
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  if [[ "$DESKTOP_APP_DETECTED" == true ]]; then
    echo -e "  ${DIM}agentzero desktop${RESET}                ${DIM}# Launch the menu bar app${RESET}"
  else
    echo -e "  ${DIM}agentzero desktop --install${RESET}      ${DIM}# Download the companion app${RESET}"
  fi
fi
echo
echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.agentzerolabs.ai/docs${RESET}"
