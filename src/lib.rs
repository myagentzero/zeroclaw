...
Examples:
  agentzero channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  agentzero cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  agentzero cron add-every 60000 'Ping heartbeat'     # every minute
  agentzero cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  agentzero cron once 2h 'Follow up on deployment'
  agentzero cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  agentzero cron update <task-id> --expression '0 8 * * *'
  agentzero cron update <task-id> --tz Europe/London --name 'Morning check'
  agentzero cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  agentzero hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  agentzero peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash AgentZero firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash AgentZero firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  agentzero peripheral flash
  agentzero peripheral flash --port /dev/cu.usbmodem12345
  agentzero peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash AgentZero firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
