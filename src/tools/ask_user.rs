...
impl Tool for AskUserTool {
    ...
    fn description(&self) -> &str {
        "Ask the user a question and wait for a reply. Use when: you need clarification, confirmation, or input from the user."
    }
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                ...
                "timeout_secs": {
                    "type": "integer",
                    "description": format!("Seconds to wait for a response. Omit to use the preferred default ({} s).", self.config.default_timeout_secs)
                },
                "channel": {
                    "type": "string",
                    "description": format!("Channel to send the question to. Omit to use the preferred default ({}).", self.config.default_channel.as_deref().unwrap_or("auto"))
                },
                "recipient": {
                    "type": "string",
                    "description": "Thread ID to send the question to and filter replies from. Omit to use the preferred default (auto-populated from context)."
                }
            },
            "required": ["question"]
        })
