...
mod tests {
    ...
    async fn file_edit_replaces_single_match() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_single");
    ...
    async fn file_edit_not_found() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_notfound");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello world")
            .await
    ...
    async fn file_edit_flexible_match_indentation_difference() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_indent");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(
            dir.join("test.txt"),
    ...
    async fn file_edit_flexible_match_tab_space_difference() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_tabs");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_flexible_match_trailing_whitespace_difference() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_trailing");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_flexible_match_collapsed_spaces() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_spaces");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_flexible_match_ambiguous_errors() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_ambiguous");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(
            dir.join("test.txt"),
    ...
    async fn file_edit_flexible_match_not_found_when_no_line_match() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_not_found");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "alpha\nbeta\n")
            .await
    ...
    async fn file_edit_prefers_exact_match_over_flexible() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_exact_preference");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(
            dir.join("test.txt"),
    ...
    async fn file_edit_flexible_match_preserves_trailing_newline_when_old_string_has_none() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_flex_no_trailing_nl");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "line one\n    line two\nline three\n")
            .await
    ...
    async fn file_edit_multiple_matches() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_multi");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_delete_via_empty_new_string() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_delete");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_allows_sensitive_file_when_configured() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_sensitive_allowed");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_rejects_empty_old_string() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_empty_old_string");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_expands_tilde_path_consistently_with_policy() {
        ...
        let target_rel = format!("agentzero_tilde_edit_{}.txt", uuid::Uuid::new_v4());
        ...
        let workspace = std::env::temp_dir().join("agentzero_test_file_edit_tilde_workspace");
    ...
    async fn file_edit_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("agentzero_test_file_edit_symlink_escape");
    ...
    async fn file_edit_blocks_symlink_target_file() {
        ...
        let root = std::env::temp_dir().join("agentzero_test_file_edit_symlink_target");
        let workspace = root.join("workspace");
        let outside = root.join("outside");
    ...
    async fn file_edit_blocks_readonly_mode() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_readonly");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello")
            .await
    ...
    async fn file_edit_blocks_when_rate_limited() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_rate_limited");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello")
            .await
    ...
    async fn file_edit_nonexistent_file() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_nofile");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_null_byte_in_path() {
        let dir = std::env::temp_dir().join("agentzero_test_file_edit_null_byte");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
