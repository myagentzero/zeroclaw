//! Flash AgentZero Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash AgentZero Nucleo firmware. Builds from firmware/nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (agentzero's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from agentzero repo root.",
            firmware_dir.display()
        );
    }

    println!("Building AgentZero Nucleo firmware...");
    ...
    println!("AgentZero Nucleo firmware flashed successfully.");
