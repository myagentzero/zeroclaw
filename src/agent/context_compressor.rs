...
pub use crate::config::ContextCompressionConfig;
...
/// Inbound attachment-kind marker prefixes that the channel parsers can emit.
/// A non-vision summarizer cannot consume these, and leaving them in would
/// leak a local filesystem path into the auxiliary `chat_with_system` payload.
...
impl ContextCompressor {
    ...
    fn fast_trim_tool_results(&self, history: &mut [ChatMessage]) -> usize {
        ...
        for msg in &mut history[protect_start..protect_end] {
            ...
            msg.content = truncate_with_ellipsis(&msg.content, max);
    ...
    pub async fn compress_if_needed(
        ...
        for _ in 0..self.config.max_passes {
            let did_compress = self
                .compress_once(history, provider, model, temperature)
                .await?;
            if did_compress {
                passes_used += 1;
            }
    ...
    pub async fn compress_on_error(
        ...
        tracing::info!(
            ...
            "context_compressor: context limit adjusted, re-compressing"
        );

        let result = self
            .compress_if_needed(history, provider, model, temperature)
            .await?;
    ...
    async fn compress_once(
        ...
        .await
        {
            ...
            Ok(Err(e)) => {
                tracing::warn!(
                    error = %e,
                    "context_compressor: summarization LLM call failed, using transcript truncation"
                );
                truncate_with_ellipsis(&transcript, self.config.summary_max_chars)
            }
            Err(_) => {
                tracing::warn!(
                    timeout_secs = self.config.timeout_secs,
                    "context_compressor: summarization timed out, using transcript truncation"
                );
                truncate_with_ellipsis(&transcript, self.config.summary_max_chars)
            }
        };

        let summary = truncate_with_ellipsis(&summary_raw, self.config.summary_max_chars);
        ...
        if let Some(ref memory) = self.memory {
            ...
            match memory
                .store(&facts_key, &summary, MemoryCategory::Daily, None)
                .await
            {
                Ok(()) => {
                    tracing::debug!(
                        message_count,
                        "context_compressor: saved compression summary to memory before discarding messages"
                    );
                }
                Err(e) => {
                    tracing::debug!(
                        error = %e,
                        "context_compressor: failed to save compression summary to memory"
                    );
                }
            }
        }
        ...
        tracing::warn!(
            ...
            "context_compressor: middle of conversation replaced with a text summary. The model \
             loses structural tool_use/tool_result pairs from this range. If this fires mid-turn \
             the model can act like it just woke up. Raise protect_last_n / protect_first_n, or \
             raise the context window, or lower threshold_ratio carefully."
        );
...
fn align_boundary_backward(messages: &[ChatMessage], idx: usize) -> usize {
    ...
    // one and any preamble assistant immediately preceding it.
...
fn build_summarizer_transcript(
    ...
    if preserve_media_markers {
        ...
        // visual content is reflected in the summary.
    ...
    truncate_owned_if_needed(strip_media_markers(&transcript), max_chars)
}
...
/// DeepSeek), embed the most recent such payload in the summary as a
...
/// to the provider passes its reasoning round-trip check.
...
mod tests {
    ...
    #[async_trait]
    impl Provider for CaptureSummarizerModelProvider {
    ...
    /// When the tail boundary lands on an assistant with `tool_calls`, the
    /// function must back up past it so the assistant travels with its
    /// `tool_result` blocks into the protected tail. Otherwise the assistant
    /// gets summarized while its results survive, creating an orphan and
    /// producing a 400 "unexpected tool_use_id" failure.
    #[test]
    ...
    fn test_build_summarizer_transcript_strips_all_attachment_kinds_for_non_vision_provider() {
        ...
        // `[IMAGE:]`. A `[PHOTO:]` or `[DOCUMENT:]` marker leaking through
        // would surface a local filesystem path in the auxiliary
        // `chat_with_system` payload and the upstream API would reject it.
    ...
    fn test_config_defaults() {
        ...
        assert!((config.threshold_ratio - 0.65).abs() < f64::EPSILON);
    ...
    /// the synthesized summary is plain text.
    #[test]
    ...
    /// When an assistant message in the compressed range carries
    /// `reasoning_content` (thinking-mode replay payload), the synthesized
    /// summary preserves it via JSON-encoded content matching
    /// `build_native_assistant_history`'s shape. Without this, providers that
    /// require reasoning round-trip reject every post-compression request.
    #[test]
    ...
    /// the most recent one survives — matching the protocol expectation
    /// that the *immediately prior* assistant turn's reasoning is replayed.
    #[test]
