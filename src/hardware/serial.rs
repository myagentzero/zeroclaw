...
//!
//! Wire protocol (AgentZero serial JSON):
//! ```text
...
/// Default baud rate for AgentZero serial devices.
...
/// Serial transport for AgentZero hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify AgentZero firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "agentzero"`.
    ///
    /// Returns `true` if a AgentZero device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "agentzero" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
