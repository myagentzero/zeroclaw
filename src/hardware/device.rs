...
pub enum DeviceKind {
    ...
    Nucleo,
    /// Unknown VID that passed the AgentZero firmware ping handshake.
    Generic,
}
...
impl DeviceRegistry {
    ...
    ///    if the device responds with AgentZero firmware.
        ...
        for info in scan_serial_devices() {
            ...
            let probe_transport = if !is_known_vid {
                ...
                if !probe.ping_handshake().await {
                    tracing::debug!(
                        port = %info.port_path,
                        "skipping unknown device: no AgentZero firmware response"
                    );
                    continue;
                }
                Some(probe)
...
impl DeviceRegistry {
    ...
    /// AgentZero firmware is alive, and re-attaches the transport.
    ///
