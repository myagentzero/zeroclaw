...
// in `~/.agentzero/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.agentzero/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(agentzero_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: agentzero_dir.join(".secret_key"),
            enabled,
        }
    }
