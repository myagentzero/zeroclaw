...
//! AgentZero to external platforms. Each channel implements the [`Channel`] trait
...
async fn describe_non_cli_approvals(
    ...
    if runtime_auto.is_empty() {
        ...
        let _ = writeln!(response, "- Auto-approve: {}", runtime_auto.join(", "));
    }
    ...
    if runtime_always.is_empty() {
        ...
        let _ = writeln!(response, "- Always-ask: {}", runtime_always.join(", "));
    }
    ...
    if session_grants.is_empty() {
        ...
        let _ = writeln!(response, "- Session grants: {}", session_grants.join(", "));
    }
    ...
    if approval_approvers.is_empty() {
        ...
        let _ = writeln!(response, "- Approvers: {}", approval_approvers.join(", "));
    }
    ...
    let _ = writeln!(response, "- NL approval mode: {}", default_mode);
    let _ = writeln!(
        response,
    ...
    if excluded.is_empty() {
        ...
        let _ = writeln!(response, "- Excluded tools: {}", excluded.join(", "));
    }
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    response.push_str(
        "Natural language: `approve tool shell` (direct mode) or `confirm` after request.\n",
    );
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.agentzero/config.toml directly");
        }
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    println!("🩺 AgentZero Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `agentzero gateway` then GET /health");
    }
...
pub async fn start_channels(
    ...
    if configured_channels.is_empty() {
        tracing::info!("No channels configured. Run `agentzero onboard` to set up channels.");
        return Ok(());
    }
